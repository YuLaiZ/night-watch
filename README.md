# 夜航杂志 Night Watch

> **感谢智谱的馈赠，本项目均使用夜间免费 GLM-5.3-Flash 模型产出。**

一份由 AI 夜班每晚出版的杂志：看天上的星、看世界此刻的街、看公有领域里的旧画、
读一篇古文，最后给自己写一部长篇连载。一夜一期，攒满一个活动期就是一整本。

- 在线阅读：<https://yulaiz.github.io/night-watch/>
- `index.html`：总目录（期号索引）
- `pages/YYYY-MM-DD.html`：每期杂志（观星台 / 世界此刻 / 历史上的今天 / 夜班美术馆 /
  夜航手记 / 今日宇宙 / 夜班书房 / 每日一词 / 谜题角 / 压轴连载《夜班编年史》），配图在 `assets/<日期>/`
- `pages/serial.html`：《夜班编年史》连读页，一章不落，一页读完

所有页面零外部依赖，克隆后双击即读。

## 运营方式

每晚 23:02 开工，约 3 小时交卷：客户端 ZCode、模型 GLM-5.3-Flash，
指挥家 YuLaiZ。一夜一期，即校即推，不限期数。
姊妹馆：[夜班棋手联赛](https://github.com/YuLaiZ/night-league)（[在线](https://yulaiz.github.io/night-league/)）·
[无用发明陈列室](https://github.com/YuLaiZ/night-inventions)（[在线](https://yulaiz.github.io/night-inventions/)）

---
夜班出品 · 2026 · 客户端 ZCode · 模型 GLM-5.3-Flash · 指挥家 YuLaiZ
